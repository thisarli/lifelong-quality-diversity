...
def main(args):
    ...
    # env_lists = [[0,1,1,1,1], [0,2,2,2,2], [2, 1, 1, 1, 1]]
    env_lists = [[1, 2, 2, 2, 2], [2, 0, 0, 0, 0], [1, 0, 0, 0, 0]]
    descriptor_path = ['2_from_1', '0_from_2', '0_from_1']
    
    num_runs = 10
    EPSILON = args.epsilon

    for model_learn_num in range(0, 3):
        for run in range(0, num_runs):
            print(f"------------------------------------------------Learn model {model_learn_num}---------------------------------------------")
            print(f"------------------------------------------------Run {run}---------------------------------------------")
            ...
            def create_new_model(s_list_transfer, dynamics_model_transfer, model_count, archive_transfer):
                ...
                log_dir = (args.log_dir + descriptor_path[model_learn_num] + '/run_' + str(run) + '/' + str(model_count))
                ...
                new_llqd = ModelBasedLLQD(args.dim_map, dim_x,
                                        ...
                                        params=px, log_dir=args.log_dir + descriptor_path[model_learn_num] + '/run_' + str(run) + '/' + str(model_count))
            ...
            env_list = env_lists[model_learn_num]
            ...
            with open(args.log_dir + descriptor_path[model_learn_num] + '/run_' + str(run) + '/envs_vs_estimates.npy', 'wb') as f:
                np.save(f, env_visited_array)
            with open(args.log_dir + descriptor_path[model_learn_num] + '/run_' + str(run) + '/envs_vs_estimates.txt', 'wb') as g:
...
# python3 run_scripts/llqd_experiment_2.py --log_dir results/experiment_2/llqd_transfer_learn/ --dump_period 500000 --epsilon 0.5 --min_model_add 200 --random_init_batch 200
