...
def collect_data(filename = "log_file.dat",):
    ...
    for variant in variant_names:
        ...
        for run, f in enumerate(files):
            ...
            if ("random_exploration" in f) or ("qd" in f):
            ...
            if "daqd" in f: # if "dyn_model_probens_archcond_2" in f
...
def interpolate_data(data, end_point=5001):
...
def plot():
    ...
    # Choose which parameter you want to plot
    ...
    #for item in ["true_neg"]:
        ...
        plt.vlines(5000, ymin=0, ymax=1000, linestyles="dashed")
...
# How to use: this plots on one diagram the median and IQ range for the parameter of interest from multiple
# log_file.dat's for the same experiment. i.e. if we run experiment_0 10x, we get 10x log_file.dat. Put these files
# into a folder, e.g. experiment_0/log_files, then specify that folder at the top of this file in variant_names
# and finally run "python analysis.py"
# You can also compare across different variants of the experiment (eg vanilla DAQD vs LLQD), by specifying the other
# variant folder path in the variant_names section above. This will plot the other variant performance in a different
# color
# Specify the parameter of interest (archive size or normalized fitness score aroundd line 180