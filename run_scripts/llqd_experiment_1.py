    ...
    for EPSILON in [0.5, 0.3, 0.7]:
        ...
        for run in range(0, num_runs):
            ...
            def create_new_model(s_list_transfer, dynamics_model_transfer, model_count, archive_transfer):
                ...
                log_dir = (args.log_dir + '/eps_' + str(EPSILON).replace('.','') + '/run_' + str(run) + '/' + str(model_count))
                ...
                new_llqd = ModelBasedLLQD(args.dim_map, dim_x,
                                          ...
                                          params=px, log_dir=args.log_dir + '/eps_' + str(EPSILON).replace('.','') + '/run_' + str(run) + '/' + str(model_count))
            ...
            log_dir = (args.log_dir + '/eps_' + str(EPSILON).replace('.', '') + '/run_' + str(run) + '/' + str(
                model_count))
            if not os.path.isdir(log_dir):
                os.makedirs(log_dir)
            ...
            with open(args.log_dir + '/eps_' + str(EPSILON).replace('.','') + '/run_' + str(run) + '/envs_vs_estimates.npy', 'wb') as f:
                np.save(f, env_visited_array)
            with open(args.log_dir + '/eps_' + str(EPSILON).replace('.','') + '/run_' + str(run) + '/envs_vs_estimates.txt', 'wb') as g:
...
# python3 run_scripts/llqd_experiment_1.py --log_dir results/experiment_1 --dump_period 500000