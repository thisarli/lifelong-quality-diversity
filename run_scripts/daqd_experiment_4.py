...
# # To retrieve env lists used in llqd counterpart (200 easy)
# env_lists = []
#
# for run in range(20):
#     env_vs_est = np.load('results/experiment_3/200_easy/run_'+str(run)+'/envs_vs_estimates.npy')
#     env_lists.append(env_vs_est[0].tolist())
#
# print(env_lists)

# Use LLQD, but strip away env detection, so it is the same as daqd
...
env_lists = [[0, 2, 2, 2, 1, 1], [2, 0, 1, 1, 1, 0], [2, 1, 0, 2, 0, 1], [0, 0, 0, 1, 1, 1], [2, 2, 0, 2, 1, 1],
             [0, 0, 2, 0, 2, 2], [0, 1, 2, 1, 2, 0], [2, 0, 2, 2, 0, 0], [0, 2, 2, 0, 0, 0], [2, 1, 1, 2, 1, 2],
             [2, 0, 2, 0, 0, 2], [1, 0, 1, 1, 0, 0], [0, 1, 0, 1, 1, 2], [2, 0, 0, 1, 2, 2], [1, 1, 1, 0, 2, 1],
             [0, 2, 2, 2, 0, 2], [1, 2, 0, 0, 2, 1], [2, 1, 1, 0, 1, 0], [2, 0, 2, 0, 0, 2], [1, 2, 2, 2, 0, 2]]
...
# This is an implementation of the main script, but with automated env selection and thus
# evaluation
...
def main(args):
    ...
    for run in range(0, len(env_lists)):
        print(f"------------------------------------------------Run {run}---------------------------------------------")
        ...
        px = \
            {
                ...
                "random_init_batch": args.random_init_batch, # 100 TODO double, half?
        ...
        # ------- START LLQD LOOP IMPLEMENTATION HERE -------
        ...
        def create_new_model(s_list_transfer, dynamics_model_transfer, model_count, archive_transfer):
            ...
            log_dir = (args.log_dir + '/run_' + str(run) + '/' + str(model_count))
            ...
            new_llqd = ModelBasedLLQD(args.dim_map, dim_x,
                                      ...
                                      params=px, log_dir=args.log_dir + '/run_' + str(run) + '/' + str(model_count))

            new_llqd.s_list = s_list_transfer
            new_llqd.dynamics_model.load_state_dict(dynamics_model_transfer.state_dict())
            # new_llqd.archive = archive_transfer  # cannot just transfer as the archive members have wrong fitness for this env
            new_llqd.gen_start_time = time.time()
            print(f"New model made; the log_dir is {new_llqd.log_dir}")

            return new_llqd, model_count

        # We will have multiple llqd objects for each new environment.
        ...
        # For batches of observations, we take the median likelihood and use this to determine which of the available
        # models is best.
        # If all are sufficiently far away (worse), create a new model object.

        # 0. Make a random list of envs to go through

        env_list = env_lists[run]
        ...
        while env_counter < len(env_list) - 1:
            ...
            # prompt = input("Are you ready to see if the switch is correct?")
            ...
            # 3. Compare the likelihoods for all models and pick the best (smallest negative number)
            ...
            #
            #     dynamics_model_transfer = model_list[transfer_learning_candidate_idx].dynamics_model  # use deepcopy?
            ...
            # 4. Do some more logic with the best model.
            # This should include the computation of likelihoods (and means and std).
            print("Entering compute_more_stuff for this best fitting model...")
            tmp_best_model.compute_more_stuff(num_cores_set=args.num_cores, max_evals=args.max_evals)
            print("Successfully computed more stuff for the best fitting model")
        ...
        print("The models we visited in order were", visited_models)

        # 6 Save the env_list and visited_list
        env_visited_array = np.array([env_list, visited_models])
        with open(args.log_dir + '/run_' + str(run) + '/envs_vs_estimates.npy', 'wb') as f:
            np.save(f, env_visited_array)
        with open(args.log_dir + '/run_' + str(run) + '/envs_vs_estimates.txt', 'wb') as g:
...
# python3 run_scripts/daqd_experiment_4.py --log_dir results/experiment_4/daqd --dump_period 500000 --epsilon 0.5 --min_model_add 200 --random_init_batch 200
