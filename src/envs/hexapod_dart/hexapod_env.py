...
class HexapodEnv:
    ...
    def update_dynamics_model(self, dynamics_model):
        self.dynamics_model = dynamics_model
        return 1
    ...
    def init_robot(self):
        ...
        robot.free_from_world([0, 0, 0.0, 0, 0, 0.2])  # place robot slightly above the floor 0.15
    ...
    def init_floor(self):
        floor_array = np.loadtxt('src/envs/hexapod_dart/floor.txt').reshape(6, 1)
        return floor_array

    def simulate(self, ctrl, sim_time, robot, render=None, video_name=None):

        floor = self.init_floor()
        ...
        simu.set_collision_detector(
            "bullet")  # DART Collision Detector is the default but does not support cylinder shapes
        ...
        simu.add_checkerboard_floor(20., 0.1, 1., floor, "floor")
        ...
        # print("We perform real sims with floor: ", floor)
    ...
    def simulate_model_ensemble(self, ctrl, sim_time, mean, disagr):
        ...
        for t in np.arange(0.0, sim_time, 1 / self.ctrl_freq):
            ...
            # if probalistic dynamics model - choose output mean or sample
            if disagr:
                pred_delta_ns, _ = self.dynamics_model.sample_with_disagreement(
                    torch.cat((self.dynamics_model._expand_to_ts_form(s), self.dynamics_model._expand_to_ts_form(a)),
                              dim=-1))
