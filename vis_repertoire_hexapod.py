...
def click_event(event, args):
    ...
    if event.button == 1:
        ...
        selected_solution = data[(data["BD x-dim"] == selected_x) & (data["BD y-dim"] == selected_y)]
        ...
        print("THIS IS THE SELECTED CONTROL SHAPE:", np.shape(selected_ctrl))
        print("THIS IS THE SELECTED CONTROL:", selected_ctrl)
...
if __name__ == "__main__":
    ...
    #For Hexapod
    data['BD x-dim']=pd.cut(x = data.iloc[:,1],
                        bins = [p/100 for p in range(101)], 
                        labels = [p for p in range(100)])
    data['BD y-dim']=pd.cut(x = data.iloc[:,2],
                        bins = [p/100 for p in range(101)],
                        labels = [p for p in range(100)])
    data['Fitness (norm)'] = (data.iloc[:, 0] + np.pi) / np.pi

    print("QD Score:", np.sum(data['Fitness (norm)']))
    print("Coverage:", len(data['Fitness (norm)']))
...
`    data['bd3_bin']=pd.cut(x = data.iloc[:,3],
    ...
    if args.plot_type == "grid":
        ...
        data.plot.scatter(x="BD x-dim",y="BD y-dim",c="Fitness (norm)",colormap="viridis", s=3, ax=ax, fontsize=14)
        ax.set_xlabel('BD x-dim', fontsize=14)
        ax.set_ylabel('BD y-dim', fontsize=14)
        plt.xlim(0,100)
        plt.ylim(0,100)
