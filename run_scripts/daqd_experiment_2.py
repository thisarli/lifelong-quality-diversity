...
def main(args):
    for run in range(10):
        for env_id in [0, 1, 2]:
            ...
            log_dir = (args.log_dir + '/run_' + str(run) + '/' + str(env_id))
            ...
            mbqd = ModelBasedQD(args.dim_map, dim_x,
                                ...
                                params=px, log_dir=args.log_dir + '/run_' + str(run) + '/' + str(env_id))

            mbqd.compute(num_cores_set=args.num_cores, max_evals=args.max_evals)  # max_evals=args.max_evals

            save_name = args.save_name

            # print("Saving torch model")
            # ptu.save_model(mbqd.dynamics_model, mbqd.log_dir + '/model.pth')
...
#  python3 run_scripts/daqd_experiment_2.py --log_dir results/experiment_2/daqd_baseline --dump_period 600000 --max_evals 800 --num_cores 8 --min_model_add 200 --random_init_batch 200
